...
import { saveData, findByEmail } from '../firebase'
...
export default function ReceptionLogin() {
  ...
  useEffect(() => {
    const seedDemoAccount = async () => {
      try {
        ...
        const existingDemo = await findByEmail('receptionists', 'reception@hospital.com')
        if (!existingDemo.success || !existingDemo.data) {
          const receptionistId = 'reception@hospital.com'.replace(/[.#$[\]]/g, '_')
          await saveData(`receptionists/${receptionistId}`, {
            ...demoAccount,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Fetch receptionist from Firebase
      const receptionistResult = await findByEmail('receptionists', formData.email)

      if (!receptionistResult.success || !receptionistResult.data) {
        setGeneralError('No receptionist account found. Please register first.')
        setLoading(false)
        return
      }

      const receptionistData = receptionistResult.data

      // Verify password
      if (receptionistData.password === formData.password) {
        // Store login session with full receptionist data
        localStorage.setItem('receptionistId', receptionistData.receptionistId)
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-green-50 to-emerald-100 p-4">
      <div className="w-full max-w-md bg-white rounded-lg shadow-lg p-8">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500">Access the reception portal</p>
        </div>
